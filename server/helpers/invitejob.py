    ...
    def run(self):
        ...
        with closing(self._database_session_maker()) as database_session:
            ...
            for db_test in db_tests:
                ...
                if test.opened_at != None and test.closed_at == None and test.invite_method == Test.INVITE_METHOD_EMAIL:

                    db_test_sessions = database_session.query(DbTestSession) \
                        .filter(DbTestSession.test_id == test.id) \
                        .filter(DbTestSession.invited_at == None) \
                        .all()
                    ...
                    for db_test_session in db_test_sessions:
                        ...
                        try:
                            ...
                            invite_sender.sendInvite(database_session, test_session, db_test_session=db_test_session, test=test)
                        except:
                            print("Error while sending mail to {}".format(test_session.email))

        print("Completed InviteJob")
