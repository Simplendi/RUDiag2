angular.module('app').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/test.html',
    "<div class=row><div class=\"page-border col-xs-12\" ng-if=\"state=='loading'\">Loading</div><div class=\"page-border col-xs-12\" ng-if=\"state=='data'\"><p>Please complete the following data to start the test.</p><form class=form-horizontal><div class=form-group><label for=name class=\"col-sm-2 control-label\">Name</label><div class=col-sm-10><input class=form-control id=name placeholder=\"Firstname Lastname\" ng-model=testSession.name></div><label for=student_id class=\"col-sm-2 control-label\">Student-ID</label><div class=col-sm-10><input class=form-control id=student_id placeholder=3123456 ng-model=testSession.student_id></div><label for=email class=\"col-sm-2 control-label\">Email</label><div class=col-sm-10><input class=form-control id=email placeholder=yourname@example.org ng-model=testSession.email></div><div class=\"col-sm-offset-2 col-sm-10\"><a href=\"\" class=\"btn btn-default\" ng-click=dataDone()>Done</a></div></div></form></div><div class=\"page-border col-xs-9\" ng-if=\"state=='answer'\"><test test=test test-session=testSession></test></div><div class=\"page-border col-xs-12\" ng-if=\"state=='done'\"><h1>{{test.title}}</h1>Your answers are submitted successfully. <span ng-if=\"test.feedback_timing=='at'\">You can see your results at {{test.feedback_at | timeFormat }} by visiting this page again.</span> <span ng-if=\"test.feedback_timing=='after'\">Your results will be available after {{test.feedback_after}} minutes by visting this page again.</span></div><div class=\"page-border col-xs-12\" ng-if=\"state=='feedback'\"><test-feedback test=test test-session=testSession></test-feedback></div><div ng-if=\"state=='answer'\" class=\"col-xs-3 side-bar\"><div ng-if=\"state=='answer'\" class=\"col-xs-3 status-block page-border\">Last saved <span second-difference ng-model=testSession.updated_at scale=minutes></span> minutes ago <a href=\"\" ng-click=submit() class=\"btn btn-default\">Submit answers</a></div></div></div>"
  );
  ...
  $templateCache.put('views/directives/choice_question.html',
    "<div contentable ng-model=data.content.question_before></div><span class=question-option ng-repeat=\"choice in data.content.choices track by $index\"><img src=/static/img/16x16.png ng-if=\"showResult&&(!answerIsRight()&&data.answers[0]!=$index||answerIsRight())\"> <img src=/static/img/tick.png ng-if=\"showResult&&!answerIsRight()&&data.answers[0]==$index\"> <img src=/static/img/16x16.png ng-if=\"showResult&&answer!=$index\"> <img src=/static/img/tick.png ng-if=\"showResult&&answerIsRight()&&answer==$index\"> <img src=/static/img/cross.png ng-if=\"showResult&&!answerIsRight()&&answer==$index\"> <input ng-if=showResult type=radio disabled ng-checked=\"answer==$index\"> <input ng-if=!showResult type=radio ng-model=$parent.$parent.answer ng-value=\"$index\"><div contentable class=question-option-content ng-model=data.content.choices[$index]></div></span><div contentable ng-model=data.content.question_after></div>"
  );


  $templateCache.put('views/directives/multiplechoice_question.html',
    "<div contentable ng-model=data.content.question_before></div><span class=question-option ng-repeat=\"choice in data.content.choices track by $index\"><img src=/admin/static/img/16x16.png ng-if=\"showResult&&answerIsRight()\"> <img src=/admin/static/img/tick.png ng-if=\"showResult&&answerIsRight()&&answer.indexOf($index)>=0\"> <img src=/admin/static/img/16x16.png ng-if=\"showResult&&answerIsRight()&&answer.indexOf($index)<0\"> <img src=/admin/static/img/tick.png ng-if=\"showResult&&!answerIsRight()&&data.answers[0].indexOf($index) >= 0\"> <img src=/admin/static/img/16x16.png ng-if=\"showResult&&!answerIsRight()&&data.answers[0].indexOf($index) < 0\"> <img src=/admin/static/img/cross.png ng-if=\"showResult&&!answerIsRight()&&answer.indexOf($index) >= 0\"> <img src=/admin/static/img/16x16.png ng-if=\"showResult&&!answerIsRight()&&answer.indexOf($index) < 0\"> <input ng-if=showResult type=checkbox disabled ng-checked=\"answer.indexOf($index) >= 0\"> <input ng-if=!showResult type=checkbox ng-click=toggleAnswerOption($index) ng-checked=\"$parent.$parent.answer.indexOf($index) >= 0\"><div contentable class=question-option-content ng-model=data.content.choices[$index]></div></span><div contentable ng-model=data.content.question_after></div>"
  );
